  ...
  return new Promise((resolve, reject) => {
    ...
    request.onload = () => {
      ...
      try {
        ...
        // Fallback: try to extract ID manually if JSON parsing fails
...
function App() {
   ...
   * Signs out the user if the token is invalid.
   */
  useEffect(() => {
   ...
   * Ensures an active session exists once the user is logged in.
   */
  useEffect(() => {
  ...
  /** Resets quiz state when moving to a new question */
  useEffect(() => {
    setAnswerInput("");
    setFeedback(null);
  ...
  /** Redeems access code and starts the initial session */
  ...
  /** Starts a completely fresh study session */
  ...
  /**
   * Upload Flow:
   * 1) Requests a short-lived upload URL from Convex.
   * 2) Uploads file via XMLHttpRequest.
   * 3) Registers document in backend and triggers AI extraction.
   */
  const handleFileUpload = async (files: File[]) => {
    ...
    for (const file of files) {
      try {
        ...
        // Extraction happens asynchronously in the background (AI Action)
  ...
  /** Generates new quiz questions based on uploaded documents */
  const handleGenerateQuiz = async () => {
    ...
    try {
      await generateQuiz({
        grantToken,
        sessionId,
        questionCount: 30, // More questions for extended training
      });
    } catch (error: unknown) {
  ...
  /** Submits user answer to the AI for evaluation */
  const handleSubmitAnswer = async (dontKnowSubmission: boolean = false) => {
    ...
    try {
      // Track time for better progress analysis
  ...
  /** Triggers comprehensive AI analysis of the current study session */
  ...
  /** Generates deep-dive content for a specific topic with learning gaps */
        ...
        <aside className="flex flex-col border-r border-cream-border bg-surface-white p-6">
          ...
          <div className="mt-auto pt-6">
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-cream-border bg-surface-white px-4 py-2.5 text-xs font-bold uppercase tracking-[0.12em] text-ink-muted transition hover:text-ink disabled:opacity-60"
            >
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          setIsRemovingDocument(document._id);
                          try {
                            await removeDocument({ grantToken, sessionId, documentId: document._id });
                          } finally {
                            setIsRemovingDocument(null);
                          }
                        }}
                        disabled={isRemovingDocument === document._id}
                        className="ml-4 rounded-full p-2 text-ink-muted transition hover:bg-cream-light hover:text-ink disabled:opacity-60"
                        aria-label={`Datei ${document.fileName} entfernen`}
                      >
                        {isRemovingDocument === document._id ? <Loader2 size={20} className="animate-spin" /> : <XCircle size={20} />}
                      </button>
                    </div>
                  ))
                )}
              ...
              <div className="w-full max-w-4xl text-center">
                {!currentQuestion && stats.totalQuestions > 0 && !feedback ? (
                  ...
                  <div className="rounded-3xl border border-cream-border bg-cream-light p-12 text-center">
                    <h2 className="mb-4 text-4xl font-bold">Starke Leistung!</h2>
                  ...
                  <>
                    ...
                    <div className="mx-auto max-w-2xl">
                      ...
                      <div className="mt-8 flex flex-col items-center gap-6">
                        ...
                        <button
                          ...
                          disabled={isSubmittingAnswer}
                          className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] text-ink-muted transition hover:text-accent disabled:opacity-60"
                        >
                          {isSubmittingAnswer ? <Loader2 size={12} className="animate-spin" /> : null}
                    ...
                    <div className="mt-32">
                      <button
                        ...
                        disabled={isAnalyzing}
                        className="inline-flex items-center gap-2 rounded-full border border-cream-border px-4 py-2 text-xs font-bold uppercase tracking-[0.15em] text-ink-muted transition hover:bg-red-50 hover:text-red-500 hover:border-red-100 disabled:opacity-60"
                      >
          ...
          {stage === "analysis" && (
            <section>
              <header className="mb-6">
                ...
                <h1 className="text-3xl font-bold tracking-tight">Lernanalyse</h1>
                <p className="mt-2 text-sm text-ink-secondary">
                  Wähle ein Thema mit Lücken für eine gezielte Vertiefung oder starte eine neue Sitzung.
                </p>
              </header>
...
/**
 * Component to display current progress status in the sidebar.
 */
...
/**
 * Component to display Key Performance Indicators (KPIs) in the analysis view.
 */
...
/**
 * Helper function to render document extraction status in a human-readable format.
 */
